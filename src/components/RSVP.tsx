...
export default function RSVP() {
  ...
  function validate(): FormErrors {
    ...
    if (!form.note.trim())
      e.note = "Vui lòng để lại lời chúc cho cô dâu &amp; chú rể.";
    return e;
  }
  ...
  async function submitRsvp() {
    ...
    try {
      if (!ENDPOINT) {
        ...
        console.warn(
          "[RSVP] VITE_RSVP_ENDPOINT not set — submission not persisted.",
          payload
        );
      } else {
            ...
            >
              ...
              {status === "error" && (
                <div className="rsvp-banner rsvp-banner--error" role="alert">
                  Có lỗi khi gửi. Vui lòng thử lại hoặc liên hệ trực tiếp với
                  chúng mình.
                  <button
                    type="button"
                    className="rsvp-banner-retry"
                    onClick={retry}
                  >
                    Thử lại
                  </button>
